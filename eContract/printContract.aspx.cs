...
namespace eContract
{
    public partial class printContract : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string _userType = Request["userType"];
            Login _login = new Login(_userType);

            //string _parentType = "student";
            //_login.StudentCode = "6103092";
            //string _studentId = "STD256103342";
            string _parentType = _login.UserType;// type: M, F, student
            string _studentId = _login.StudentId;

            if (_userType == "STUDENT" && !Request.IsAuthenticated)
            {
            ...
            switch (_action)
            {
                case "printContract":
                    ...
                    printContractStd(_studentId, _login.StudentCode);
                    break;
                case "printConsent":
                    //Response.Write("printConsent"); B
                    printGarrantee(_studentId, _parentType, _login.StudentCode);
                    break;
                case "printWarrant":
                    //Response.Write("printWarrant"); C
                    printWarrant(_studentId, _parentType, _login.StudentCode);
                    break;
                case "printAll":
                    //Response.Write("printAll"); A,B,C
                    printAll(_studentId, _parentType);
                    break;
            }
        ...
        public void printContractStd(string _studentId, string _studentCode)
        {
            ...
            if (_contract == " - ")
            {
                ...
                else if (_ProgramCode == "PYPYB")
                {
                    ...
                    _cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, _signCEO, 335, _y, 0);//ชื่ออธิการบดี
                    _y = 382;
                    _cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "รักษาการแทนอธิการบดีมหาวิทยาลัยมหิดล", 335, _y, 0);//ตำแหน่งอธิการบดี
                    _cb.EndText();
                }
        ...
        #region printAll
        public void printAll(string _studentId, string _parentType)
        {

            //printContractStd(_studentId); //A
            //printGarrantee(_studentId, _parentType);//B
            //printWarrant(_studentId, _parentType);//C
            ContractInfo _ctInfo = new ContractInfo(_studentId);
            string _file1 = Myconfig.CvEmpty(_ctInfo.contractPath, " - ");
            string _file2 = Myconfig.CvEmpty(_ctInfo.garranteePath, " - ");
            string _file3 = Myconfig.CvEmpty(_ctInfo.warranPath, " - ");
            string[] fileNames = new string[] { _file1, _file2, _file3 };
        ...
        public PdfImportedPage page2 { get; set; }
    }
}